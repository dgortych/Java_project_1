...
public class LiquidMatterWagon extends BasicTruckWagon implements Liquid{
    ...
    @Override
    public void pour_our(double amount) throws Exception {
        if( this.cargo - amount < 0 ){
            throw new Exception("Materiału ciekłego jest mniej niż podana wartość");
        }
        this.cargo -= amount;
        this.gross_weight -= amount;
    }
    ...
    public String toString() {
        return "Wagon Na Materiały Ciekłe - " + super.toStringBasic() +
                ", gęstość=  " + Math.round(this.density * 100.0) / 100.0 +
